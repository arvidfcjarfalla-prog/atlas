...
Full-pipeline skill for non-trivial tasks. Follows the Workflow Engine pipeline from CLAUDE.md.
...
1. **Clarify** — Surface assumptions before building.
   ...
   - After the user answers: save the Q&A to `tmp/experience/{domain}.md`. This creates a flywheel — each task adds context, so future tasks need fewer questions.
...
5. **Test** — Run `pnpm typecheck && pnpm test`. Deterministic verification only.
...
8. **Review** — Run `/review` (reviewer command, fresh context, changed files only). If reviewer returns ISSUES_FOUND → spawn resolver (fresh context, sees BOTH original code + review critique). Resolver either FIXES or DECLINES each issue with reasoning. Re-review resolved output. Max 3 loops.
