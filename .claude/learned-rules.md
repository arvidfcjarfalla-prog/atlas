...
[NEXT-TYPES] P1 ALWAYS rm `apps/web/.next/types/app/<route>/` after deleting an App Router route file, or `pnpm typecheck` fails with `TS2307: Cannot find module '.../route.js'`. Next 14 generates type shims in `.next/types/` that do not get cleaned up when the source route disappears. Hit this deleting `/api/ai/edit-map` and `/api/ai/case-memory` in Batch C (2026-04-13). The app-level test suite doesn't catch it — only `tsc --noEmit` does, because Vitest skips the `.next/` tree.
